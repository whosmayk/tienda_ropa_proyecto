...
if(isset($_POST['delete'])){
    try {
        $conn->query("SET @usuario_app = '".$_SESSION['usuario']."'");
        $stmt = $conn->prepare("DELETE FROM color WHERE id_color = ?");
        $stmt->execute([$_POST['delete_id']]);
        header("Location: colores.php?msg=eliminado");
        exit;
    } catch (Exception $e) {
        $error = "No se puede eliminar este color porque tiene prendas asociadas.";
    }
}
?>
<!DOCTYPE html>
        ...
        <div class="row">
            ...
            <div class="col-md-4 mb-4">
                <div class="card card-custom">
                    <div class="card-header card-header-custom">Nuevo Color</div>
                    <div class="card-body">
                        <form method="POST">
                            <div class="mb-3">
...
if (puede_gestionar_catalogo()) {
    echo "<button class='btn btn-sm btn-warning' 
        onclick='abrirEditar({$row['id_color']}, \"".addslashes($row['nombre'])."\")'>
        Editar
    </button>
